...
class Server:
    ...
    def __init__(self):
        ...
        self.by = ServerInfo.Info('by').get_info()
        self.auth = ServerInfo.Info('auth').get_info()
        self.mail = ServerInfo.Info('mail').get_info()
        self.remode = ServerInfo.Info('remode').get_info()
        self.ytchannel = ServerInfo.Info('ytchannel').get_info()
        self.date = ServerInfo.Info('date').get_info()
        self.conf = ServerConfig.conf
        self.noyes = [ru('No'), ru('Yes')]
        self.version = [ru('Default'), ru('HTTP/1.0'), ru('HTTP/1.1')]
        self.method = [ru('CONNECT'),
         ru('GET'),
         ru('POST'),
         ru('DELETE'),
         ru('CONNECT'),
    ...
    def about(self, title = ''):
        self.info = []
        self.info.append('[ %s ]%s\n' % (title, '=' * (self.long - len(title) - 6)))
        self.info.append('Name : %s\n' % self.name)
        self.info.append('Version : %s\n' % self.ver)
        self.info.append('Update : %s\n' % self.date)
        self.info.append('Develovers : %s\n' % self.auth)
        self.info.append('Email : %s\n' % self.mail)
        self.info.append('Youtube : %s\n' % self.ytchannel)
    ...
    def show(self):
        sys.stderr.write(self.about('SIMPLE SERVER TELKOMSEL'))
        time.sleep(1)
        sys.stderr.write(self.config('Configuration'))
        time.sleep(2)
        sys.stderr.write(self.log('      ========WARNING!========'))
        time.sleep(3)
        sys.stderr.write(self.log('PLEASE! SUBSCRIBE CHANNEL IbonXalopa'))
